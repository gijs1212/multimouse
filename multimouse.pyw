...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    ...
    def _build_combi(self, root):
        ...
        ttk.Button(calib, text=" " + tr("restart_close"),
                   command=lambda: self._calib_key("restart_close_app", tr("restart_close"))).grid(row=6, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("restart_search"),
                   command=lambda: self._calib_key("restart_searchbar", tr("restart_search"))).grid(row=7, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("full_calibration"),
                   command=self._full_calibration_combi).grid(row=8, column=0, padx=6, pady=(6,0), sticky="ew")
    ...
    def _start_combi(self):
        ...
        if not all(required):
            messagebox.showerror(
                tr("error_calib"),
                "Combi kalibratie incompleet (Rode blokjes + Foto 1 sender + Foto 2 sender + Foto reply + Versturen 1 + Versturen 2).",
            ); return
