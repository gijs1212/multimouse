...
def update_autosnap_startup(enable: bool):
    ...
    if not start_dir:
        return
    src = BASE_PATH() / "autosnap_startup.pyw"
    dst = start_dir / "autosnap_startup.pyw"
    try:
        if enable:
            shutil.copy(src, dst)
        else:
...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    ...
    def _build_ui(self):
        ...
        self.startup_chk = ttk.Checkbutton(wrap, text=tr("windows_startup"), variable=self.startup_enabled, command=self._toggle_startup)
        self.startup_chk.grid(row=2, column=0, sticky="w", pady=(0,5))
        ttk.Button(wrap, text=tr("save_settings"), command=self.save_combined).grid(row=3, column=0, sticky="e", pady=(0,5))
