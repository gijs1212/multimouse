...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    ...
    def _build_combi(self, root):
        ...
        ttk.Button(calib, text=" " + tr("badge_points"),
                   command=self._calib_responder_badges).grid(row=0, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" Foto (sender)",
                   command=lambda: self._calib_key("foto1", "Foto (sender)")).grid(row=1, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" Foto (reply)",
                   command=lambda: self._calib_key("foto_reply", "Foto (reply)")).grid(row=2, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("send_to"),
                   command=lambda: self._calib_key("verstuur_na_foto", tr("send_to"))).grid(row=3, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("send"),
                   command=lambda: self._calib_key("verzend", tr("send"))).grid(row=4, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("restart_close"),
                   command=lambda: self._calib_key("restart_close_app", tr("restart_close"))).grid(row=5, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("restart_search"),
                   command=lambda: self._calib_key("restart_searchbar", tr("restart_search"))).grid(row=6, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("full_calibration"),
                   command=self._full_calibration_combi).grid(row=7, column=0, padx=6, pady=(6,0), sticky="ew")
    ...
    def _start_responder(self):
        need = [self.cfg.get("foto_reply"), self.cfg.get("verzend")]
    ...
    def _start_combi(self):
        ...
        if not all(required):
            messagebox.showerror(tr("error_calib"),
                                 "Combi kalibratie incompleet (Rode blokjes + Foto sender + Foto reply + Versturen 1 + Versturen 2)."); return
