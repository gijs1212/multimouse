...
def color_in_radius(x, y, target, radius=RADIUS_DEFAULT, tol=30):
    return any_match_in_radius(x, y, lambda rgb: _color_close(rgb, target, tol), radius)
...
DEFAULT_SNAP_CONFIG = {
    ...
    "responder_badges": [None]*8,         # badge pos+kleur
    ...
    def _ensure_calibrated(self, items):
        ...
        for key, label in items:
            ...
            if key == "responder_badges":
                if not any(((isinstance(v, dict) and v.get("pos")) or v) for v in (val or [])):
    ...
    def _calib_responder_badges(self):
        ...
        for i in range(8):
            ...
            try:
                ...
                r, g, b = (0, 0, 0)
            pts.append({"pos": [x, y], "color": [r, g, b]})
        while len(pts) < 8:
            pts.append(None)
        ...
        try:
            while self.responder_running.is_set():
                ...
                if entry:
                    ...
                    if pos:
                        x, y = pos
                        color_hit = color and color_in_radius(x, y, color, RADIUS_DEFAULT)
                        red_hit = (not color) and red_in_radius(x, y, RADIUS_DEFAULT)
                        if (color_hit or red_hit) and (idx not in reacted_this_round):
    ...
    def _combi_send_reply(self):
        ...
        for entry in badges:
            ...
            if not pos:
                continue
            x, y = pos
            hit = (color and color_in_radius(x, y, color, RADIUS_DEFAULT)) or (
                (not color) and red_in_radius(x, y, RADIUS_DEFAULT))
            if hit:
