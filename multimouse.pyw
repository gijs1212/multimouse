...
mss = _require_module("mss", "mss")  # type: ignore
try:
...
LANGS = {
    "nl": {
        ...
        "run_as_admin": "Start als administrator",
...
def _color_close(rgb, target, tol=50):
    """Return True if the Euclidean distance between two RGB colours is within tol."""
...
def ensure_admin():
    ...
    try:
        ...
        _fatal(f"Kon niet als administrator starten: {e}")
    sys.exit(0)
...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    def __init__(self, master, get_lang, set_lang, save_combined, load_combined):
        ...
        self.run_as_admin = tk.BooleanVar(value=bool(self.cfg.get("run_as_admin")))
    ...
    def _load_settings(self):
        ...
        self.run_as_admin.set(bool(self.cfg.get("run_as_admin")))
    ...
    def _toggle_run_admin(self):
        self.cfg["run_as_admin"] = bool(self.run_as_admin.get())
        save_snap_config(self.cfg)
    ...
    def _open_settings(self):
        ...
        ctk.CTkCheckBox(tab_start, text=tr("run_as_admin"), variable=self.run_as_admin,
                        command=self._toggle_run_admin).pack(anchor="w", padx=10, pady=8)
    ...
    def _calib_scanner_color(self):
        ...
        try:
            with mss.mss() as sct:
                ...
                rgb = Image.frombytes("RGB", (1, 1), shot.rgb).getpixel((0, 0))
        except Exception:
            return
