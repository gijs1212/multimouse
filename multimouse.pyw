...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    ...
    def _build_combi(self, root):
        ...
        ttk.Button(calib, text=" " + tr("color_scanner"),
                   command=self._calib_scanner_color).grid(row=7, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("close_snap"),
                   command=lambda: self._calib_key("responder_close_snap", tr("close_snap"))).grid(row=8, column=0, padx=6, pady=6, sticky="ew")
    ...
    def _respond_sequence(self, badge_xy):
        ...
        with self.ui_lock:
            ...
            if self.close_before_reply.get() and self.cfg.get("responder_close_snap"):
                # Sluit de geopende snap, klik weer op badge en ga door
