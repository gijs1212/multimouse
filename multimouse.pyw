...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    def __init__(self, master, get_lang, set_lang, save_combined, load_combined):
        ...
        extra_modes = self.cfg.get("combi_extra_people_modes", ["normal", "normal"])
        self.combi_extra_person_mode_vars = [tk.StringVar(value=extra_modes[i] if i < len(extra_modes) else "normal") for i in range(2)]
        extra_names = self.cfg.get("combi_extra_people_names", ["", ""])
        self.combi_extra_person_name_vars = [tk.StringVar(value=extra_names[i] if i < len(extra_names) else "") for i in range(2)]
        ...
        for var in self.combi_extra_person_vars + self.combi_extra_person_mode_vars + self.combi_extra_person_name_vars + self.combi_extra_person_scroll_vars:
    ...
    def _load_settings(self):
        ...
        for i, var in enumerate(self.combi_extra_person_vars):
            ...
            self.combi_extra_person_mode_vars[i].set(extra_modes[i] if i < len(extra_modes) else "normal")
            self.combi_extra_person_name_vars[i].set(extra_names[i] if i < len(extra_names) else "")
    ...
    def _save_combi_extra_people(self):
        ...
        self.cfg["combi_extra_people_modes"] = [var.get() for var in self.combi_extra_person_mode_vars]
        self.cfg["combi_extra_people_names"] = [var.get() for var in self.combi_extra_person_name_vars]
    ...
    def _build_combi(self, root):
        ...
        ttk.Button(calib, text=" Persoon 9/10 (optioneel)",
                   command=self._calib_combi_extra_people).grid(row=7, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" Snap zoekbalk personen",
                   command=lambda: self._calib_key("snap_people_searchbar", "Snap zoekbalk personen")).grid(row=8, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" Scrollpunt persoon 9/10",
                   command=self._calib_combi_extra_scroll_points).grid(row=9, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("send"),
                   command=lambda: self._calib_key("verzend", tr("send"))).grid(row=10, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("send_reply"),
                   command=lambda: self._calib_key("verzend_reply", tr("send_reply"))).grid(row=11, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("color_scanner"),
                   command=self._calib_scanner_color).grid(row=12, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("restart_close"),
                   command=lambda: self._calib_key("restart_close_app", tr("restart_close"))).grid(row=13, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("restart_search"),
                   command=lambda: self._calib_key("restart_searchbar", tr("restart_search"))).grid(row=14, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" Spotlight",
                   command=lambda: self._calib_key("close_spotlight", "Spotlight")).grid(row=15, column=0, padx=6, pady=6, sticky="ew")
        ttk.Button(calib, text=" " + tr("full_calibration"),
                   command=self._full_calibration_combi).grid(row=16, column=0, padx=6, pady=(6,0), sticky="ew")
        ...
        for i in range(2):
            row = 5 + i
            person_no = i + 9
            frame = ttk.Frame(opts)
            frame.grid(row=row, column=0, columnspan=2, sticky="ew", pady=2)
            frame.columnconfigure(4, weight=1)
            ttk.Checkbutton(frame, text=f"Persoon {person_no} meesturen", variable=self.combi_extra_person_vars[i]).grid(row=0, column=0, sticky="w")
            ttk.Radiobutton(frame, text="Normaal", variable=self.combi_extra_person_mode_vars[i], value="normal").grid(row=0, column=1, padx=6, sticky="w")
            ttk.Radiobutton(frame, text="Zoeken", variable=self.combi_extra_person_mode_vars[i], value="search").grid(row=0, column=2, padx=6, sticky="w")
            ttk.Radiobutton(frame, text="Scroll", variable=self.combi_extra_person_mode_vars[i], value="scroll").grid(row=0, column=3, padx=6, sticky="w")
            ttk.Label(frame, text="Naam").grid(row=0, column=4, padx=(8, 2), sticky="w")
            ttk.Entry(frame, textvariable=self.combi_extra_person_name_vars[i], width=16).grid(row=0, column=5, sticky="ew")
            ttk.Label(frame, text="Scroll").grid(row=0, column=6, padx=(8, 2), sticky="w")
            ttk.Entry(frame, textvariable=self.combi_extra_person_scroll_vars[i], width=6).grid(row=0, column=7, sticky="w")
    ...
    def _build_story(self, root):
        ...
        ttk.Button(
            calib,
            text=" " + tr("send_to"),
            command=lambda: self._calib_key("story_send_to", tr("send_to")),
        ...
        ).grid(row=3, column=0, columnspan=2, padx=6, pady=(6,0), sticky="ew")
        ...
        try:
            with self.ui_lock:
                ...
                for idx in persons:
                    ...
                    if not p:
                        continue
                    if use_combi_extra_search and idx >= 8 and idx < 10:
                        extra_idx = idx - 8
                        mode = extra_modes[extra_idx]
                        if mode == "search":
                            self.move_then_click(searchbar)
                            if search_text:
                                pyautogui.press("backspace", presses=len(search_text), interval=0.01)
                            search_text = extra_names[extra_idx]
                            if search_text:
                                pyautogui.typewrite(search_text, interval=0.01)
                                time.sleep(self.action_delay_var.get())
                        elif mode == "scroll":
    ...
    def _start_combi(self):
        ...
        if self.combi_send_on_start.get():
            ...
            for i, enabled in enumerate(self.cfg["combi_extra_people"]):
                if not enabled:
                    continue
                mode = self.cfg["combi_extra_people_modes"][i]
                if mode == "search":
                    if not self.cfg.get("snap_people_searchbar"):
                        search_missing.append("Snap zoekbalk personen")
                    if not self.cfg["combi_extra_people_names"][i].strip():
                        search_missing.append(f"Naam persoon {i+9}")
                elif mode == "scroll":
                    if i >= len(scroll_points) or not scroll_points[i]:
                        scroll_missing.append(f"Scrollpunt persoon {i+9}")
