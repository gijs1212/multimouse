...
class AutoSnapWindow(ctk.CTkToplevel, MiniMixin):
    def __init__(self, master, get_lang, set_lang, save_combined, load_combined):
        ...
        self.current_combi_time_sel = None
    ...
    def _build_combi(self, root):
        ...
        self.combi_times_list.bind("<<ListboxSelect>>", self._combi_on_time_select)
    ...
    def _combi_add_time(self):
        raw = self.combi_new_time_var.get()
        t = parse_hhmm(raw)
        if not t:
            messagebox.showerror("Time", "Invalid time format. Use HH:MM"); return
        self.combi_new_time_var.set(t)
    ...
    def _combi_refresh_times(self):
        ...
        self.current_combi_time_sel = self._combi_selected_time()

    def _combi_on_time_select(self, event=None):
        if self.current_combi_time_sel:
            self._combi_save_people_for_selected_time(self.current_combi_time_sel)
        self._combi_load_people_for_selected_time()
        self.current_combi_time_sel = self._combi_selected_time()
    ...
    def _combi_save_people_for_selected_time(self, sel=None):
        sel = sel or self._combi_selected_time()
        if not sel:
            return
        self.combi_time_people[sel] = [bool(v.get()) for v in self.combi_people_vars]
        self.cfg["combi_time_people"] = self.combi_time_people
        save_snap_config(self.cfg)
    ...
    def _get_current_combi_persons(self, t_str: str | None = None):
        if t_str:
            ...
            if sel and sel in self.combi_time_people:
                ...
                mask = [bool(v.get()) for v in self.combi_people_vars]
    ...
    def _combi_sender_scheduler(self):
        while self.combi_running.is_set():
            ...
            persons = self._get_current_combi_persons(t_str)
            if persons:
                self._run_sender_once(persons)
            time.sleep(0.2)
